                ...
                if ($cursor === '{' && ($next === '%' || $next === '{' || $next === '#')) {
                    if ($next !== '#') {
                        ...
                        while ($next = $this->state->incrementStart()->getStart()) {
                            if ($next === '#' && $this->state->incrementStart()->getStart() === '}') {
                                $this->token->setOpcode(Compiler::PHVOLT_T_IGNORE);
                                return 0;
                            } elseif ($next === "\n") {
                                $this->state->incrementActiveLine();
                        ...
                        return self::PHVOLT_SCANNER_RETCODE_ERR;
                    }
...
vv35:
            ...
            $this->token->setValue(substr($this->state->getRawBuffer(), $start, $this->state->getCursor() - $start));
            return 0;
vv36:
			$this->state->incrementStart();
...
vv81:
            $this->state->setStart($this->state->getMarker());
			switch ($vvaccept) {
                case 0:
                    goto vv5;
                case 1:
...
vv82:
            ...
            $this->token->setValue(substr($this->state->getRawBuffer(), $start, $this->state->getCursor() - $start - 1));
            return 0;
vv84:
			$vvch = $this->state->incrementStart()->getStart();
			switch ($vvch) {
